# Plan Bổ Sung Lessons cho Các Nghề Thiếu Nội Dung

**Ngày lập:** 2026-07-22  
**Mục tiêu:** Enrichment các career profiles có < 5 lessons → tối thiểu 6-8 lessons/career

---

## 🎯 Tiêu Chí Ưu Tiên

### Nhóm D (Pending - Cần nội dung gốc)
- ✅ **Compliance Officer** (6/6 bài AML/KYC hoàn tất)
- 🟡 **ESG Analyst** (3→6 bài)
- 🟡 **Insurance Actuarial** (4→7 bài)

### Nhóm Khác (Đang đơn giản quá)
- 🟡 Careers có 4 lessons hoặc ít hơn
- 🟡 Careers mà các lesson hiện tại khá generic (chỉ link tới 4 bài cũ)

---

## 📋 Chi Tiết Từng Career

### 1. ESG Analyst 🌍 (3 → 6 bài)

**Hiện tại:** rui-ro-la-gi, modern-portfolio-theory, enterprise-value

**Cần thêm 3 bài:**
1. **"ESG là gì & Tại sao nó quan trọng"** (khái niệm + framework GRI/SASB)
   - Định nghĩa ESG, 3 trụ cột (Environmental, Social, Governance)
   - Tại sao nhà đầu tư quan tâm (long-term value, risk mitigation)
   - Framework GRI (Global Reporting Initiative) vs SASB (Sustainability Accounting Standards Board)
   
2. **"Cách đánh giá ESG của một doanh nghiệp"** (methodology + scoring)
   - 10-15 chỉ số ESG phổ biến cho từng ngành
   - Cách tính ESG score
   - Red flags trong báo cáo ESG (vague commitment, greenwashing)
   
3. **"ESG Investing: Từ screening đến portfolio construction"** (ứng dụng thực tế)
   - ESG screening (ngoài trừ vs best-in-class)
   - Impact investing vs SRI (Socially Responsible Investing)
   - Green bonds & sustainable finance instruments

**Priority:** HIGH (lĩnh vực mới nổi, cạnh tranh cao)

---

### 2. Insurance Actuarial 🧮 (4 → 7 bài)

**Hiện tại:** bao-hiem, bao-hiem-bao-ve-tai-san, rui-ro-vo-no, rui-ro-lai-suat

**Cần thêm 3 bài:**
1. **"Bảo hiểm là gì & Mô hình kinh doanh"** (nền tảng + quản lý rủi ro)
   - Quy trình định phí (premium calculation)
   - Dự phòng nghiệp vụ (reserve) & khả năng thanh toán
   - Lợi ích của bảo hiểm cho doanh nghiệp & cá nhân
   
2. **"Actuarial Science: Xác suất & thống kê ứng dụng"** (toán học)
   - Bảng tử suất (mortality table), bảng sống sót (survival curve)
   - Tính toán giá trị hiện tại kỳ vọng (present value of future benefits)
   - Mô hình định phí bảo hiểm nhân thọ vs phi nhân thọ
   
3. **"Solvency II & Quy định bảo hiểm"** (compliance)
   - Tiêu chí Solvency II / Quy định của BTC Việt Nam
   - Risk Management Framework for Insurers
   - SAC (Solvency Capital Requirement) & MCR (Minimum Capital Requirement)

**Priority:** HIGH (niche + specialized knowledge needed)

---

### 3. Wealth Manager / Financial Planner 💼 (4 → 6 bài)

**Hiện tại:** gia-tri-thoi-gian-cua-tien, modern-portfolio-theory, rui-ro-la-gi, ke-hoach-thua-ke

**Cần thêm 2 bài:**
1. **"Hoạch định tài chính toàn diện: 6 bước"** (methodology)
   - Phân tích tình hình tài chính hiện tại
   - Xác định mục tiêu dài hạn (hưu trí, con cái, mua nhà)
   - Asset allocation phù hợp khẩu vị rủi ro
   
2. **"Tư vấn tài chính cho các giai đoạn cuộc đời"** (application)
   - Accumulation (tích lũy): 20-40 tuổi
   - Consolidation (ổn định): 40-55 tuổi
   - Distribution (rút chi): 55+ tuổi

**Priority:** MEDIUM (career phát triển, khách hàng cá nhân tăng)

---

### 4. Risk Management Specialist 🛡️ (4 → 6 bài)

**Hiện tại:** rui-ro-la-gi, quan-ly-rui-ro-lai-suat-doanh-nghiep, quan-ly-rui-ro-ty-gia-doanh-nghiep-xnk, modern-portfolio-theory, rui-ro-vo-no

**Cần thêm 1-2 bài:**
1. **"Value at Risk (VaR) & Stress Testing"** (định lượng)
   - Khái niệm VaR, cách tính toán
   - Stress testing scenarios
   - Backtesting & validation
   
2. **"Basel III & Regulatory Capital Requirements"** (compliance)
   - Tier 1 & Tier 2 capital
   - RWA (Risk Weighted Assets) calculation
   - Liquidity Coverage Ratio (LCR) & Net Stable Funding Ratio (NSFR)

**Priority:** MEDIUM (technical knowledge, less beginner-friendly)

---

### 5. Valuation Specialist 🏢 (4 → 6 bài)

**Hiện tại:** enterprise-value, dinh-gia-tai-san-rong, dupont-analysis, fcf-deep-dive

**Cần thêm 2 bài:**
1. **"Định giá bất động sản & Tài sản vô hình"** (beyond DCF)
   - Income Approach vs Cost Approach vs Market Approach
   - Goodwill & Intangible Assets valuation
   - Brand valuation methods
   
2. **"Valuation Report & Tính hợp lý của định giá"** (practice)
   - Cấu trúc một valuation report
   - Sensitivity analysis & scenario planning
   - Giải thích định giá cho kiểm toán & cơ quan quản lý

**Priority:** MEDIUM (specialized + high-value work)

---

### 6. FP&A (Financial Planning & Analysis) 🧮 (4 → 6 bài)

**Hiện tại:** fpa-la-gi-vai-tro-doanh-nghiep, xay-dung-ngan-sach-doanh-nghiep, du-bao-tai-chinh-rolling-forecast, phan-tich-variance-thuc-te-vs-ke-hoach, kpi-tai-chinh-doanh-nghiep-chon-dung-chi-so

**Cần thêm 1-2 bài:**
1. **"Driver-based Budgeting & Scenario Planning"** (advanced)
   - Ngân sách dựa trên drivers (bán hàng forecast → chi phí)
   - What-if analysis (What if revenue ↓ 10%?)
   - Rolling forecast & reforecasting timing
   
2. **"Dashboard & KPI Visualization"** (tools + soft skills)
   - Power BI / Tableau trong FP&A
   - KPI design & tracking
   - Storytelling với dữ liệu tài chính

**Priority:** MEDIUM-HIGH (practical + in-demand tool skill)

---

### 7. Credit Officer 💳 (4 → 6 bài)

**Hiện tại:** no-tot-no-xau, vay-tien-giau-hay-pha-san, don-bay-tai-chinh, credit-rating, rui-ro-vo-no

**Cần thêm 1 bài:**
1. **"Quy trình thẩm định tín dụng: CIC score & 5C framework"** (methodology)
   - 5C framework (Character, Capacity, Capital, Collateral, Conditions)
   - CIC score interpretation
   - Quyết định phê duyệt / từ chối

**Priority:** MEDIUM (practical, reduce risky loans)

---

### 8. Investment Analyst 🎯 (4 → 6 bài)

**Hiện tại:** enterprise-value, dupont-analysis, roic, roic-phan-2, fcf-deep-dive

**Cần thêm 1 bài:**
1. **"Investment Thesis & Research Report Structure"** (practice)
   - Cách viết investment thesis mạch lạc
   - Cấu trúc research report (Executive Summary, Investment Case, Risks, Valuation)
   - Pitch trước investment committee

**Priority:** MEDIUM (career progression skill)

---

### 9. Stockbroker 📈 (4 → 6 bài)

**Hiện tại:** thi-truong-tai-chinh, thanh-khoan-la-gi, rui-ro-la-gi, loi-nhuan-ky-vong, modern-portfolio-theory

**Cần thêm 2 bài:**
1. **"Phân tích kỹ thuật (Technical Analysis) cho môi giới"** (TA tools)
   - Support/Resistance levels
   - Moving averages, RSI, MACD
   - Khi nào nên recommend buy/sell
   
2. **"Quản lý ký quỹ (Margin) & Xử lý margin call"** (risk management)
   - Ký quỹ là gì, cơ chế hoạt động
   - Tính toán margin call
   - Xử lý khách hàng khi bị margin call

**Priority:** MEDIUM-HIGH (practical, reduce customer churn)

---

### 10. Treasury Officer 💼 (4 → 6 bài)

**Hiện tại:** treasury-management-quan-ly-dong-tien, quan-ly-rui-ro-lai-suat-doanh-nghiep, quan-ly-rui-ro-ty-gia-doanh-nghiep-xnk, cau-truc-von-toi-uu-cho-doanh-nghiep, case-xuat-khau-phong-ho-ty-gia

**Cần thêm 1 bài:**
1. **"Hedging instruments & FX derivatives"** (advanced)
   - Forward contracts, futures, options, swaps
   - Cách sử dụng để hedge rủi ro
   - Accounting for hedges (VAS guidance)

**Priority:** MEDIUM (technical + niche market)

---

## 📊 Tóm tắt Workload

| Priority | Careers/Subject | Trạng thái | Bài đã viết |
|----------|---------|-------------|--------|
| **HIGH** | Compliance ✅ | Done | 3 bài |
| **HIGH** | ESG (3), Actuarial (4) | ⚠️ Chưa làm | 0 bài |
| **MEDIUM-HIGH** | Wealth, FP&A, Broker | ✅ Done | 6 bài |
| **MEDIUM** | Risk, Valuation, Treasury, Analyst, Credit | ✅ Done | 7 bài |
| **CFA** | Economics (Business Cycles, Fiscal, Trade, FX, Market Structures) | ✅ Done | 5 bài |
| **Backlog** | 22 nghề còn 3-4 bài (xem Phase 5) | ⚠️ Chưa làm | 0 bài |
| **Backlog** | CFA Alternatives (đóng khung lại) | ⚠️ Chưa làm | 0 bài |
| **Backlog** | CFA vignette/item-set format | ⚠️ Chưa làm (feature, không phải content) | - |
| **Backlog** | Chặng Behavioral Finance | ⚠️ Chưa làm (cần thiết kế cấu trúc trước) | - |
| **Total đã viết** | | | **21 bài** |

---

## 🎬 Giai đoạn Thực Hiện

### Phase 1 (2026-07-22): HIGH Priority ✅ DONE
- ✅ Compliance Officer (3 bài AML/KYC) - commit `1ed1466`
- ⚠️ ESG Analyst - **vẫn còn 3 bài, chưa viết thêm** (không phải "done" như bảng cũ ghi nhầm)
- ⚠️ Insurance Actuarial - **vẫn còn 4 bài, chưa viết thêm**

### Phase 2 (2026-07-22): MEDIUM-HIGH ✅ DONE
- ✅ Wealth Manager (2 bài: hoạch định tài chính toàn diện, theo giai đoạn cuộc đời) - commit `00e4b7f`
- ✅ FP&A (2 bài) - commit `02c50e8`
- ✅ Stockbroker (2 bài: phân tích kỹ thuật, margin call) - commit `02c50e8`

### Phase 3 (2026-07-22): MEDIUM ✅ DONE
- ✅ Risk Management (2 bài: VaR & Stress Testing, Basel III) - commit `95680ef`
- ✅ Valuation (2 bài: định giá BĐS/tài sản vô hình, valuation report) - commit `95680ef`
- ✅ Treasury (1 bài: hedging & FX derivatives) - commit `95680ef`
- ✅ Investment Analyst (1 bài: investment thesis) - commit `95680ef`
- ✅ Credit Officer (1 bài: 5C framework) - commit `95680ef`

### Phase 4 - CFA Economics (2026-07-22) ✅ DONE
Subject mỏng nhất so với trọng số thi (6-9% weight, chỉ 4 bài cũ) - đã bổ sung 5 bài chuẩn CFA Level I, bám sát Learning Outcome Statements chính thức (Business Cycles, Fiscal Policy, International Trade, FX & Capital Flows, Market Structures). Slug prefix `cfa-economics-*`, theo đúng pattern chất lượng của `cfa-ethics-*`.

### Phase 5 (chưa làm) - Nghề nghiệp còn mỏng
Audit lại cho thấy **22 nghề** vẫn chỉ có 3-4 bài liên quan (`relatedLessonSlugs`), thấp hơn nhiều so với các nghề đã mở rộng ở Phase 1-3:
`esg-analyst` (3), `fintech-product-finance` (3), `insurance-actuarial` (4), `accountant` (4), `fund-manager` (4), `quant` (4), `ir-specialist` (4), `pmi-specialist` (4), `pe-vc-analyst` (4), `internal-audit` (4), `retail-banking-rm` (4), `real-estate-finance` (4), `family-office-advisor` (4), `macro-research-analyst` (4), `personal-financial-advisor` (4), `financial-coach` (4), `insurance-financial-advisor` (4), `household-finance-planner` (4), `etf-fund-specialist` (4), `fixed-income-trader` (4), `consumer-credit-analyst` (4), `management-accountant` (4).

### Phase 6 (chưa làm) - CFA Alternatives
9 bài hiện tại đúng chủ đề (LBO, VC, PE, Hedge Fund, Commodity, Real Estate, Crypto) nhưng viết dưới dạng case-study cá nhân, chưa đóng khung theo CFA Level I curriculum (không có nhãn `cfa-alternatives-*`, thiếu liên hệ trực tiếp Learning Outcome Statements).

### Phase 7 (chưa làm) - Định dạng câu hỏi vignette/item-set cho CFA
Đề thi CFA thật dùng item set (1 đoạn tình huống + 3-6 câu hỏi ăn theo) - app hiện chỉ có câu hỏi đơn lẻ độc lập (`CfaQuizSidebar.tsx`). Cần thiết kế type mới (`CfaVignette` với `scenario` + `questions[]`) và thử nghiệm ở vài module tiêu biểu trước khi nhân rộng.

### Phase 8 (chưa làm) - Chặng Behavioral Finance riêng
Hiện chưa có chặng/track riêng cho tài chính hành vi (behavioral finance) - cần xác định gắn vào track nào (personal, professional, hay bonus) và thiết kế cấu trúc bài học trước khi viết nội dung.

---

## 🔍 Quality Checklist per Lesson

Mỗi bài phải có:
- [ ] Title rõ ràng (Day XX: ...)
- [ ] Subtitle 1 dòng
- [ ] Opening Question + 4 options (1 đúng)
- [ ] Explanation cho opening Q
- [ ] 3 Quiz questions (level tăng dần)
- [ ] 5+ Key Takeaways
- [ ] 1 Practice Prompt
- [ ] Summary + Common Mistake + Action
- [ ] Application section
- [ ] 6-10 Content sections (lead → heading → paragraph → list)
- [ ] Real World Example

---

## 🎯 KPI Tracking

- [ ] Tất cả HIGH priority: 100% by end of Month 1
- [ ] Tất cả MEDIUM-HIGH: 100% by end of Month 2
- [ ] Tất cả MEDIUM: phần lớn by end of Month 3
- **Target:** 19 bài mới trong 3 tháng (6-7 bài/tháng)
