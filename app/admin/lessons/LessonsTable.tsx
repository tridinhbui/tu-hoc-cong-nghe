                ...
                <tr key={lesson.id}>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-mono text-ink-faint w-10">
                        #{lesson.id}
                      </span>
  ...
  return (
      ...
      footer={
        <>
          <button
            onClick={onClose}
            disabled={saving}
            className="px-4 py-2 text-sm font-semibold rounded-lg border border-line-strong text-ink-body hover:bg-surface disabled:opacity-50"
          >
            {tl.cancelButton}
          </button>
          <button
            onClick={handleSave}
            disabled={saving}
            className="px-4 py-2 text-sm font-bold rounded-lg bg-stone-900 hover:bg-stone-800 dark:bg-stone-100 dark:hover:bg-white text-ink-invert disabled:opacity-50"
          >
    ...
    >
      <div className="space-y-4">
        <div>
          <label className="text-xs font-bold text-ink-muted uppercase tracking-wide">
            {tl.titleLabel}
          </label>
          <input
            value={title}
            onChange={(e) => setTitle(e.target.value)}
            className="mt-1 w-full px-3 py-2 text-sm rounded-lg border border-line-strong bg-white dark:bg-stone-800 text-ink focus:outline-none focus:border-stone-500"
          />
        </div>
        <div>
          <label className="text-xs font-bold text-ink-muted uppercase tracking-wide">
          ...
          <textarea
            ...
            rows={2}
            className="mt-1 w-full px-3 py-2 text-sm rounded-lg border border-line-strong bg-white dark:bg-stone-800 text-ink focus:outline-none focus:border-stone-500 resize-none"
          />
        </div>
        <div>
          <label className="text-xs font-bold text-ink-muted uppercase tracking-wide">
          ...
          <input
            ...
            className="mt-1 w-full px-3 py-2 text-sm rounded-lg border border-line-strong bg-white dark:bg-stone-800 text-ink focus:outline-none focus:border-stone-500 disabled:opacity-50"
          />
        </div>
