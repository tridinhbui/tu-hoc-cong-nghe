...
export const lessonOverrides = {
  ...
  "working-capital-van-hanh": patch({
    quiz: [
      q(
        "CCC = DIO + DSO - DPO. Đo:",
        ["Doanh thu", "Thời gian từ chi tiền mua hàng đến nhận tiền bán hàng - ngắn = tốt", "Tiền vào ra", "Không định nghĩa"],
        1,
      ...
      q(
        "Startup CCC dài vì:",
        ["Quản lý tệ", "Không lịch sử → nhà cung cấp không cho nợ + khách chậm trả", "Vì luật pháp", "Vì không vốn"],
        1,
  ...
  // ── Lessons 1217-1223 ────────────────────────────────────────────────
  //
  // This block shipped with all 21 of its quiz questions missing the
  // `explanation` field, so a learner who answered wrong was told only that
  // they were wrong. Explanations added below.
  //
  // Three questions needed more than an explanation:
  //
  //  - value-at-risk Q1 taught an outright misconception. It marked "VaR 95%
  //    = $3M is more dangerous than VaR 99% = $5M" correct, but for one
  //    portfolio VaR *always* rises with the confidence level - the two
  //    numbers are two points on the same loss distribution and cannot be
  //    ranked against each other. Question rewritten.
  //  - hedging-instruments Q1's correct option was garbled mid-sentence
  //    ("Options: quyền (không bắt buộc), Future: bắt buộc"). Rewritten as
  //    two clean axes: where it trades, and whether it obliges.
  //  - Typos in two distractors ("Tiền lãi lầu ngân hàng", "Giá cua tòa nhà").
  //
  // Option order here is authored for readability; lib/lesson-quiz-balance.js
  // redistributes the answer positions afterwards.

  "value-at-risk-var-stress-testing": patch({
    quiz: [
      q(
        "Một danh mục có VaR 1 ngày ở mức tin cậy 95% là 3 triệu USD, và VaR 1 ngày ở mức 99% là 5 triệu USD. Kết luận nào đúng?",
        [
        ...
        1,
        "Với cùng một danh mục, VaR luôn tăng khi mức tin cậy tăng - cắt sâu hơn vào đuôi phân phối lỗ thì ngưỡng lỗ phải lớn hơn. Nên 3 triệu ở 95% và 5 triệu ở 99% là nhất quán, không phải hai mức rủi ro để đem so hơn kém. Điều quan trọng hơn: VaR chỉ nói lỗ VƯỢT ngưỡng bao nhiêu lần, không nói vượt BAO XA. Ngày tệ nhất trong 1% kia có thể lỗ 8 triệu, cũng có thể 40 triệu - VaR im lặng về chuyện đó, và đó chính là lý do phải dùng thêm Expected Shortfall và stress test."
      ),
      q(
        "Stress test là gì và khác VaR như thế nào?",
        [
  ...
  "basel-iii-regulatory-capital-requirements": patch({
    quiz: [
      q(
        "Sự khác biệt giữa Tier 1 capital và Tier 2 capital là gì?",
        [
  ...
  "dinh-gia-bat-dong-san-tai-san-vo-hinh": patch({
    quiz: [
      q(
        ...
        1,
        "Ba cách này trả lời ba câu hỏi khác nhau về cùng một tài sản: Income Approach hỏi 'nó đẻ ra bao nhiêu tiền', Market Approach hỏi 'người ta vừa trả bao nhiêu cho cái tương tự', Cost Approach hỏi 'xây lại tốn bao nhiêu'. Một định giá tử tế chạy cả ba rồi giải thích vì sao chúng lệch nhau - chênh lệch giữa Income và Market thường tiết lộ thị trường đang kỳ vọng gì mà dòng tiền hiện tại chưa thể hiện. Một con số đứng đơn độc rất khó bảo vệ khi bị chất vấn."
      ),
      q(
        "Income Approach định giá bất động sản dựa trên điều gì?",
        [
        ...
        2,
        "Income Approach chính là DCF khoác áo bất động sản: lấy NOI (doanh thu cho thuê trừ chi phí vận hành, chưa trừ lãi vay và khấu hao) rồi hoặc chia cho cap rate để ra giá trị, hoặc chiết khấu dòng NOI nhiều năm về hiện tại. Vì thế nó nhạy nhất với đúng hai giả định: tỷ lệ lấp đầy và cap rate. Cap rate nhích 0,5 điểm phần trăm có thể làm định giá đổi cả chục phần trăm - đó là lý do báo cáo định giá bất động sản bắt buộc phải có bảng độ nhạy, không phải cho đẹp."
      ),
      q(
        "Goodwill là gì?",
        [
  ...
  "valuation-report-tinh-hop-ly-dinh-gia": patch({
    quiz: [
      q(
        "Executive Summary trong valuation report nên chứa gì?",
        [
        ...
        2,
        "Executive Summary viết cho người chỉ đọc đúng một trang - thường là CFO, hội đồng đầu tư, hoặc thẩm phán. Nó phải trả lời trọn ba câu: giá trị bao nhiêu (nên là một KHOẢNG, không phải một số duy nhất), bằng phương pháp nào, dựa trên giả định nào. Đủ để người đọc ra quyết định mà không cần lật phần kỹ thuật. Nhồi chi tiết phương pháp vào đây là biến nó thành phần thân bài thứ hai và làm hỏng đúng chức năng của nó."
      ),
      q(
        "Tại sao Sensitivity Analysis quan trọng?",
        [
        ...
        0,
        "Định giá là một hàm số của các giả định, nên một con số đơn lẻ luôn tỏ ra chính xác hơn thực tế. Bảng độ nhạy hai chiều (thường là WACC × tốc độ tăng trưởng dài hạn) phơi bày điều đó: giá trị chạy trong khoảng nào, và quan trọng hơn - GIẢ ĐỊNH NÀO thực sự lái kết quả. Nghịch lý là nó khiến báo cáo đáng tin hơn chứ không yếu đi, vì người đọc thấy bạn biết rõ chỗ nào mong manh. Báo cáo giấu độ nhạy luôn là báo cáo bị đánh sập đầu tiên khi có tranh chấp."
      ),
      q(
        "Khi một valuation report bị tranh cãi (ví dụ trong phiên tòa), điều gì được xem xét trước tiên?",
        [
        ...
        1,
        "Trong tranh chấp, không ai chứng minh được con số cuối là 'đúng' - giá trị hợp lý vốn dĩ là một ước lượng, không phải sự thật quan sát được. Cái bị mổ xẻ là QUY TRÌNH: phương pháp có phù hợp với loại tài sản không, giả định có nguồn và có nhất quán với nhau không, các giao dịch so sánh có thật sự tương đương không, ngày định giá có đúng không. Một định giá 50 triệu USD lập luận chặt chẽ sẽ đứng vững trước một định giá 50 triệu USD không giải thích nổi vì sao chọn WACC 9%."
      ),
    ],
  }),
  ...
  "investment-thesis-research-report-structure": patch({
    quiz: [
      q(
        "Các phần chính của một investment thesis là gì?",
        [
      ...
      q(
        ...
        2,
        "Trình tự này đi từ KẾT LUẬN xuống bằng chứng, vì người đọc chuyên nghiệp đọc theo đúng thứ tự đó và thường dừng giữa chừng. Mỗi phần có một nhiệm vụ riêng: ngành để định khung, công ty để nêu lợi thế cạnh tranh, định giá để ra con số, rủi ro để chứng minh bạn đã tự phản biện, khuyến nghị để nói rõ hành động và giá mục tiêu. Báo cáo chỉ có giá mục tiêu mà không có đường dẫn tới nó thì không ai đặt lệnh theo được."
      ),
    ],
  }),
  ...
  "hedging-instruments-fx-derivatives": patch({
    quiz: [
      q(
        "Sự khác biệt giữa Forward, Futures và Options là gì?",
        [
      ...
      q(
        ...
        0,
        "Interest Rate Swap không chuyển tiền gốc, chỉ hoán đổi DÒNG LÃI tính trên một số tiền danh nghĩa. Công dụng thực tế là khớp lại cấu trúc nợ với cấu trúc doanh thu: doanh nghiệp đang vay thả nổi nhưng có doanh thu ổn định sẽ trả cố định / nhận thả nổi để chốt cứng chi phí lãi vay, khỏi phải đoán lãi suất; ngân hàng dùng chiều ngược lại để cân bằng bảng cân đối. Lưu ý quan trọng: swap không làm rủi ro biến mất, nó chỉ chuyển rủi ro sang bên có khẩu vị ngược lại - và tạo ra rủi ro đối tác mới."
      ),
      q(
        "Call option trên USD là gì?",
        [
