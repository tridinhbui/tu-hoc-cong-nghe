...
export const lessonOverrides = {
  "khau-hao-co-ban": patch({
    quiz: [
      q(
        "Báo cáo kết quả kinh doanh (P&L) trả lời câu hỏi nào?",
        ["Công ty đang nợ bao nhiêu?", "Công ty có gì?", "Công ty lãi hay lỗ trong kỳ?", "Công ty có bao nhiêu tiền mặt?"],
        2,
        "P&L (Income Statement) cho biết doanh thu trừ đi chi phí = lợi nhuận ròng của kỳ."
      ),
      q(
        "P&L là báo cáo của khoảng thời gian nào?",
        ["Thời điểm cụ thể (như 31/12)", "Khoảng thời gian (quý, năm) - từ ngày này đến ngày kia", "Không có thời gian cụ thể", "Liên tục"],
        1,
        "P&L là báo cáo hiệu quả vận hành trong một chu kỳ (tháng, quý, năm), khác với bảng cân đối (thời điểm)."
      ),
      q(
        "COGS (Chi phí vốn hàng bán) nằm ở phần nào của P&L?",
        ["Ở dưới cùng (chi phí vận hành)", "Nằm ngay sau doanh thu (để tính Gross Profit)", "Không nằm trên P&L", "Ở cuối cùng"],
        1,
        "Cấu trúc P&L: Revenue - COGS = Gross Profit - Operating Expenses = Operating Income - Interest & Tax = Net Income."
      ),
      q(
        "Chi phí trả trước (Prepaid Insurance) được ghi nhận trên P&L năm nay bao nhiêu?",
        ["Toàn bộ chi phí trả trước", "Chỉ phần bảo hiểm đã được sử dụng trong năm nay", "Không ghi nhận gì", "Một nửa"],
        1,
        "Accrual accounting: chi phí khi dùng, không khi trả. Nên P&L ghi lần lần theo mức sử dụng."
      ),
      q(
        "Nếu P&L cho thấy công ty lãi 100 triệu nhưng dòng tiền âm, nguyên nhân có thể là?",
        [
        ...
        1,
        "P&L ≠ Cash Flow. P&L ghi nhận lợi nhuận kế toán, nhưng dòng tiền thực tế bị âm do chi đầu tư tài sản cố định mới (CapEx không trừ ngay vào P&L) hoặc tiền bị khóa trong tồn kho và khoản phải thu chưa thu được tiền."
      ),
      q(
        "Scenario: Công ty bán 1.000 sản phẩm với giá 100k/sản phẩm, COGS = 50k/sản phẩm. Gross Profit bằng?",
        ["50 triệu", "100 triệu", "50 tỷ", "Không thể xác định"],
        0,
        "Revenue = 1.000 × 100k = 100 triệu. COGS = 1.000 × 50k = 50 triệu. Gross Profit = 100 - 50 = 50 triệu."
      ),
      q(
        "Nếu Operating Income = 200 triệu, lãi vay = 50 triệu, thuế = 30 triệu. Net Income bằng?",
        ["200 triệu", "170 triệu", "120 triệu", "Không thể xác định"],
        2,
        "Net Income = Operating Income - Interest - Taxes = 200 - 50 - 30 = 120 triệu."
      )
    ]
  }),
  "working-capital-van-hanh": patch({
    quiz: [
      q(
        "CCC = DIO + DSO - DPO. Đo:",
        ["Doanh thu, và nguyên tắc này luôn áp dụng giống nhau cho mọi loại hình doanh nghiệp không phân biệt quy mô hay ngành nghề", "Thời gian từ chi tiền mua hàng đến nhận tiền bán hàng - ngắn = tốt", "Tiền vào ra, đây là cách tiếp cận phổ biến nhất được hầu hết các công ty lớn trên thị trường hiện nay áp dụng", "Không định nghĩa, và con số này thường được xác định cố định theo quy định chung của ngành kế toán - tài chính"],
        1,
        "CCC ngắn = công ty không phải chờ lâu = dòng tiền tốt."
      ),
      q(
        "CCC = 10 ngày:",
        ["Xấu, quá ngắn", "Tốt, rất ngắn = không phải chờ = dòng tiền tốt", "Không quyết định được", "Phụ thuộc doanh số"],
        1,
        "CCC ngắn = ít vốn lưu động = dòng tiền mạnh. Amazon ~5 ngày, startup 60+ ngày."
      ),
      q(
        "Để giảm chu kỳ chuyển đổi tiền mặt (CCC), doanh nghiệp nên áp dụng chiến lược nào?",
        [
        ...
        1,
        "Công thức CCC = DIO + DSO - DPO. Để giảm CCC (giúp giải phóng tiền mặt nhanh hơn), doanh nghiệp cần kết hợp bán hàng nhanh (↓DIO), thu tiền khách nhanh (↓DSO) và tận dụng thời gian trả chậm từ nhà cung cấp (↑DPO)."
      ),
      q(
        "Startup CCC dài vì:",
        ["Quản lý tệ, và nguyên tắc này luôn áp dụng giống nhau cho mọi loại hình doanh nghiệp không phân biệt quy mô hay ngành nghề", "Không lịch sử → nhà cung cấp không cho nợ + khách chậm trả", "Vì luật pháp, đây là cách tiếp cận phổ biến nhất được hầu hết các công ty lớn trên thị trường hiện nay áp dụng", "Vì không vốn, và con số này thường được xác định cố định theo quy định chung của ngành kế toán - tài chính"],
        1,
        "Startup CCC dài = cần vốn lưu động lớn = thiếu tiền dù lãi."
      ),
      q(
        "CCC tăng từ 30 lên 40 ngày, ảnh hưởng:",
        ["Doanh thu giảm", "Cần vốn lưu động thêm ~10 ngày doanh thu/ngày", "Không ảnh hưởng", "Lợi nhuận tăng"],
        1,
        "CCC tăng = cần tiền mặt thêm = dòng tiền bị thắt chặt."
      ),
      q(
        "JIT inventory giúp giảm CCC bằng:",
        ["Không giúp", "Giảm DIO bằng cách chỉ mua hàng khi cần", "Tăng giá", "Giảm chất lượng"],
        1,
        "JIT = DIO ~0 = tiền mặt nhả ra nhanh = CCC ngắn."
      ),
      q(
        "Startup ưu tiên tối ưu CCC hay tăng lãi:",
        ["Tỉ tăng lãi", "Tối ưu CCC trước (sống sót), sau tăng lãi", "Tỉ tối ưu CCC", "Không quan trọng"],
        1,
        "Lãi nhưng thiếu tiền → phá sản. Sống sót trước, lãi sau."
      )
    ]
  }),
  "discontinued-operations": patch({
    ...
    quiz: [
      ...
      q(
        ...
        ["Doanh nghiệp rất khỏe, và nguyên tắc này luôn áp dụng giống nhau cho mọi loại hình doanh nghiệp không phân biệt quy mô hay ngành nghề", "Chỉ là may mắn ngẫu nhiên, đây là cách tiếp cận phổ biến nhất được hầu hết các công ty lớn trên thị trường hiện nay áp dụng", "Cần soi lại chất lượng capital allocation và tính bền vững của lợi nhuận", "Không có ý nghĩa gì, và con số này thường được xác định cố định theo quy định chung của ngành kế toán - tài chính"],
        2,
  ...
  "market-fair-value": patch({
    ...
    quiz: [
      ...
      q(
        ...
        ["Có, luôn luôn đúng, và nguyên tắc này luôn áp dụng giống nhau cho mọi loại hình doanh nghiệp không phân biệt quy mô hay ngành nghề", "Không - cần so sánh cùng tốc độ tăng trưởng và chất lượng lợi nhuận trước khi kết luận công ty nào thực sự rẻ hơn", "Không thể so sánh hai công ty khác nhau, đây là cách tiếp cận phổ biến nhất được hầu hết các công ty lớn trên thị trường hiện nay áp dụng", "Chỉ đúng nếu cùng vốn hóa thị trường, và con số này thường được xác định cố định theo quy định chung của ngành kế toán - tài chính"],
        1,
  ...
  "vingroup-cash-flow": patch({
    ...
    quiz: [
      ...
      q(
        ...
        ["Không đáng lo, chỉ là biến động ngắn hạn, và nguyên tắc này luôn áp dụng giống nhau cho mọi loại hình doanh nghiệp không phân biệt quy mô hay ngành nghề", "Cần soi kỹ chất lượng lợi nhuận - lợi nhuận báo cáo có thể không chuyển hóa thành tiền mặt thật", "Doanh nghiệp chắc chắn đang gian lận, đây là cách tiếp cận phổ biến nhất được hầu hết các công ty lớn trên thị trường hiện nay áp dụng", "Đây luôn là dấu hiệu tốt vì đang đầu tư mạnh, và con số này thường được xác định cố định theo quy định chung của ngành kế toán - tài chính"],
        1,
  ...
  "enterprise-value": patch({
    ...
    quiz: [
      ...
      q(
        ...
        ["Vì EBITDA luôn chính xác hơn lợi nhuận ròng, và nguyên tắc này luôn áp dụng giống nhau cho mọi loại hình doanh nghiệp không phân biệt quy mô hay ngành nghề", "Vì EV/EBITDA trung lập với cấu trúc vốn - so sánh được các công ty dù vay nợ nhiều hay ít khác nhau, còn P/E bị ảnh hưởng bởi lãi vay và thuế", "Vì EBITDA không cần báo cáo tài chính, đây là cách tiếp cận phổ biến nhất được hầu hết các công ty lớn trên thị trường hiện nay áp dụng", "Vì P/E chỉ dùng được cho ngân hàng, và con số này thường được xác định cố định theo quy định chung của ngành kế toán - tài chính"],
        1,
  ...
  "cap-rate": patch({
    ...
    quiz: [
      ...
      q(
        ...
        ["Vì văn phòng trung tâm rủi ro thấp hơn nên nhà đầu tư chấp nhận lợi suất thấp hơn để đổi lấy sự ổn định và thanh khoản cao hơn", "Vì kho bãi luôn có NOI cao hơn, và nguyên tắc này luôn áp dụng giống nhau cho mọi loại hình doanh nghiệp không phân biệt quy mô hay ngành nghề", "Vì văn phòng trung tâm không tính cap rate, đây là cách tiếp cận phổ biến nhất được hầu hết các công ty lớn trên thị trường hiện nay áp dụng", "Vì cap rate chỉ phụ thuộc diện tích, và con số này thường được xác định cố định theo quy định chung của ngành kế toán - tài chính"],
        0,
  ...
  "operating-leverage": patch({
    ...
    quiz: [
      ...
      q(
        ...
        ["Công ty gia công vì chi phí biến đổi khó cắt giảm, và nguyên tắc này luôn áp dụng giống nhau cho mọi loại hình doanh nghiệp không phân biệt quy mô hay ngành nghề", "SaaS thường chịu tổn thương lợi nhuận nặng hơn vì chi phí cố định vẫn phải trả dù doanh thu giảm, trong khi công ty variable cost cao có thể cắt giảm chi phí theo doanh thu", "Cả hai tổn thương như nhau, đây là cách tiếp cận phổ biến nhất được hầu hết các công ty lớn trên thị trường hiện nay áp dụng", "Không doanh nghiệp nào bị ảnh hưởng vì suy thoái chỉ ảnh hưởng giá cổ phiếu, và con số này thường được xác định cố định theo quy định chung của ngành kế toán - tài chính"],
        1,
  ...
  "income-affiliates-jv": patch({
    ...
    quiz: [
      q(
        ...
        ["Vì đó là lợi nhuận từ phần sở hữu, không phải sales của core business", "Vì equity income luôn âm, và nguyên tắc này luôn áp dụng giống nhau cho mọi loại hình doanh nghiệp không phân biệt quy mô hay ngành nghề", "Vì không có dòng tiền, đây là cách tiếp cận phổ biến nhất được hầu hết các công ty lớn trên thị trường hiện nay áp dụng", "Vì là cash accounting, và con số này thường được xác định cố định theo quy định chung của ngành kế toán - tài chính"],
        0,
      ...
      q(
        ...
        ["Coi đây là tín hiệu core business đang tăng trưởng bền vững, và nguyên tắc này luôn áp dụng giống nhau cho mọi loại hình doanh nghiệp không phân biệt quy mô hay ngành nghề", "Tách riêng equity income khỏi lợi nhuận từ hoạt động lõi để đánh giá xem sự tăng trưởng đó có lặp lại được không", "Bỏ qua vì equity income luôn nhỏ, đây là cách tiếp cận phổ biến nhất được hầu hết các công ty lớn trên thị trường hiện nay áp dụng", "Tự động kết luận công ty đang gian lận, và con số này thường được xác định cố định theo quy định chung của ngành kế toán - tài chính"],
        1,
  ...
  "interim-comprehensive-income": patch({
    ...
    quiz: [
      ...
      q(
        ...
        ["Công ty đang gian lận báo cáo, và nguyên tắc này luôn áp dụng giống nhau cho mọi loại hình doanh nghiệp không phân biệt quy mô hay ngành nghề", "Hoạt động kinh doanh chính vẫn có lãi, nhưng giá trị vốn chủ đang bị bào mòn bởi biến động tỷ giá của tài sản/nợ ở nước ngoài - một rủi ro cần theo dõi dù chưa hiện thực hóa", "Net Income sẽ tự động giảm theo OCI, đây là cách tiếp cận phổ biến nhất được hầu hết các công ty lớn trên thị trường hiện nay áp dụng", "Không có ý nghĩa gì vì OCI không quan trọng, và con số này thường được xác định cố định theo quy định chung của ngành kế toán - tài chính"],
        1,
  ...
  "transfer-pricing": patch({
    ...
    quiz: [
      ...
      q(
        ...
        ["Vì họ muốn kiểm soát chất lượng sản phẩm, và nguyên tắc này luôn áp dụng giống nhau cho mọi loại hình doanh nghiệp không phân biệt quy mô hay ngành nghề", "Vì định giá nội bộ sai lệch có thể được dùng để chuyển lợi nhuận từ nước thuế cao sang nước thuế thấp, làm giảm số thuế phải nộp một cách nhân tạo", "Vì transfer pricing chỉ áp dụng cho công ty công nghệ, đây là cách tiếp cận phổ biến nhất được hầu hết các công ty lớn trên thị trường hiện nay áp dụng", "Vì luật kế toán quốc tế cấm giao dịch nội bộ, và con số này thường được xác định cố định theo quy định chung của ngành kế toán - tài chính"],
        1,
  ...
  "maple-leaf-leverage": patch({
    ...
    quiz: [
      ...
      q(
        ...
        ["Vì thép luôn tệ hơn bán lẻ, và nguyên tắc này luôn áp dụng giống nhau cho mọi loại hình doanh nghiệp không phân biệt quy mô hay ngành nghề", "Vì EBITDA của doanh nghiệp chu kỳ có thể sụt mạnh khi chu kỳ đảo chiều, khiến tỷ lệ 3x có thể vọt lên rất cao chỉ trong 1-2 năm, trong khi EBITDA doanh nghiệp phòng thủ ổn định hơn nhiều", "Vì thép có nợ bằng ngoại tệ, đây là cách tiếp cận phổ biến nhất được hầu hết các công ty lớn trên thị trường hiện nay áp dụng", "Không có sự khác biệt nào đáng kể, và con số này thường được xác định cố định theo quy định chung của ngành kế toán - tài chính"],
        1,
  ...
  "tesla-cash-flow": patch({
    ...
    quiz: [
      ...
      q(
        ...
        ["Có, luôn luôn là dấu hiệu xấu cần bán cổ phiếu ngay, và nguyên tắc này luôn áp dụng giống nhau cho mọi loại hình doanh nghiệp không phân biệt quy mô hay ngành nghề", "Không tự động xấu - cần xem công ty có đang đầu tư đúng vào tăng trưởng tương lai hay không, và liệu công ty có đủ nguồn vốn (huy động vốn, vay) để duy trì trong giai đoạn này", "Không quan trọng vì net income mới là chỉ số duy nhất cần nhìn, đây là cách tiếp cận phổ biến nhất được hầu hết các công ty lớn trên thị trường hiện nay áp dụng", "Chỉ xấu nếu công ty niêm yết ở Mỹ, và con số này thường được xác định cố định theo quy định chung của ngành kế toán - tài chính"],
        1,
  ...
  "dupont-analysis": patch({
    ...
    quiz: [
      ...
      q(
        ...
        ["Công ty B vì có đòn bẩy nên tăng trưởng nhanh hơn, và nguyên tắc này luôn áp dụng giống nhau cho mọi loại hình doanh nghiệp không phân biệt quy mô hay ngành nghề", "Công ty A thường bền vững hơn vì lợi nhuận cao đến từ hiệu quả kinh doanh thực sự, không phụ thuộc vào mức nợ có thể trở thành rủi ro khi lãi suất tăng hoặc dòng tiền gặp khó khăn", "Cả hai chất lượng như nhau vì ROE bằng nhau, đây là cách tiếp cận phổ biến nhất được hầu hết các công ty lớn trên thị trường hiện nay áp dụng", "Không thể so sánh nếu không biết ngành nghề, và con số này thường được xác định cố định theo quy định chung của ngành kế toán - tài chính"],
        1,
  ...
  "dividend": patch({
    ...
    quiz: [
      ...
      q(
        ...
        ["Không có rủi ro vì cổ tức không đổi luôn tốt, và nguyên tắc này luôn áp dụng giống nhau cho mọi loại hình doanh nghiệp không phân biệt quy mô hay ngành nghề", "Payout ratio (tỷ lệ chia so với FCF) đang tăng dần đến mức không bền vững, và công ty có thể buộc phải cắt giảm cổ tức đột ngột khi FCF không còn đủ bù đắp", "Cổ đông sẽ được lợi nhiều hơn, đây là cách tiếp cận phổ biến nhất được hầu hết các công ty lớn trên thị trường hiện nay áp dụng", "Giá cổ phiếu chắc chắn sẽ tăng, và con số này thường được xác định cố định theo quy định chung của ngành kế toán - tài chính"],
        1,
  ...
  "walmart-earnings": patch({
    ...
    quiz: [
      ...
      q(
        ...
        ["Không thể xảy ra, biên lợi nhuận thấp luôn dẫn đến ROIC thấp, và nguyên tắc này luôn áp dụng giống nhau cho mọi loại hình doanh nghiệp không phân biệt quy mô hay ngành nghề", "Có thể xảy ra nếu vòng quay tài sản (asset turnover) đủ cao để bù lại - bán được nhiều hàng hơn trên cùng lượng vốn đầu tư có thể thắng cả khi margin từng đơn vị thấp hơn", "Chỉ có thể nếu công ty đó không có nợ, đây là cách tiếp cận phổ biến nhất được hầu hết các công ty lớn trên thị trường hiện nay áp dụng", "ROIC không liên quan đến biên lợi nhuận, và con số này thường được xác định cố định theo quy định chung của ngành kế toán - tài chính"],
        1,
  ...
  "inventory-turnover": patch({
    ...
    quiz: [
      ...
      q(
        ...
        ["Không có gì đáng lo vì doanh thu vẫn tăng, và nguyên tắc này luôn áp dụng giống nhau cho mọi loại hình doanh nghiệp không phân biệt quy mô hay ngành nghề", "Doanh thu tăng có thể chỉ đến từ giảm giá xả hàng tồn cũ, trong khi hàng mới nhập có nguy cơ lỗi mốt trước khi bán được - cần xem kỹ biên lợi nhuận gộp có giảm theo không", "DIO tăng luôn là dấu hiệu tốt cho ngành thời trang, đây là cách tiếp cận phổ biến nhất được hầu hết các công ty lớn trên thị trường hiện nay áp dụng", "Chỉ cần quan tâm đến doanh thu, không cần nhìn DIO, và con số này thường được xác định cố định theo quy định chung của ngành kế toán - tài chính"],
        1,
  ...
  "post-ipo-dividend": patch({
    ...
    quiz: [
      ...
      q(
        ...
        ["Luôn tích cực vì cổ đông thích tiền mặt ngay, và nguyên tắc này luôn áp dụng giống nhau cho mọi loại hình doanh nghiệp không phân biệt quy mô hay ngành nghề", "Có thể là phân bổ vốn kém hiệu quả - nếu công ty còn dự án ROIC cao hơn chi phí vốn, giữ lại để tái đầu tư thường tạo giá trị dài hạn lớn hơn cho cổ đông so với chia cổ tức sớm", "Không quan trọng vì cổ tức không ảnh hưởng giá trị công ty, đây là cách tiếp cận phổ biến nhất được hầu hết các công ty lớn trên thị trường hiện nay áp dụng", "Chỉ nên đánh giá dựa trên phản ứng giá cổ phiếu ngắn hạn, và con số này thường được xác định cố định theo quy định chung của ngành kế toán - tài chính"],
        1,
  ...
  "disney-pixar-ma": patch({
    ...
    quiz: [
      ...
      q(
        ...
        ["Vì cost synergy không cần thực hiện gì cả, và nguyên tắc này luôn áp dụng giống nhau cho mọi loại hình doanh nghiệp không phân biệt quy mô hay ngành nghề", "Vì cost synergy nằm trong tầm kiểm soát nội bộ của doanh nghiệp (sa thải, đóng cửa trùng lặp), còn revenue synergy phụ thuộc vào phản ứng của khách hàng bên ngoài - khó dự đoán và kiểm soát hơn nhiều", "Vì revenue synergy luôn lớn hơn cost synergy, đây là cách tiếp cận phổ biến nhất được hầu hết các công ty lớn trên thị trường hiện nay áp dụng", "Vì cost synergy chỉ áp dụng cho công ty nhỏ, và con số này thường được xác định cố định theo quy định chung của ngành kế toán - tài chính"],
        1,
  ...
  "nvidia-cash-securities": patch({
    ...
    quiz: [
      ...
      q(
        ...
        ["Không cần hỏi gì vì cash nhiều luôn tốt, và nguyên tắc này luôn áp dụng giống nhau cho mọi loại hình doanh nghiệp không phân biệt quy mô hay ngành nghề", "Liệu ban lãnh đạo có đang thiếu chiến lược phân bổ vốn rõ ràng, khiến vốn cổ đông bị 'chôn' trong tài khoản thay vì tạo thêm giá trị qua đầu tư, mua lại cổ phiếu hay cổ tức", "Công ty chắc chắn sắp phá sản, đây là cách tiếp cận phổ biến nhất được hầu hết các công ty lớn trên thị trường hiện nay áp dụng", "Cash dự trữ không liên quan đến cổ đông, và con số này thường được xác định cố định theo quy định chung của ngành kế toán - tài chính"],
        1,
  ...
  "fpt-cfo-cash": patch({
    ...
    quiz: [
      ...
      q(
        ...
        ["Buyback luôn tốt hơn cổ tức trong mọi trường hợp, và nguyên tắc này luôn áp dụng giống nhau cho mọi loại hình doanh nghiệp không phân biệt quy mô hay ngành nghề", "Phụ thuộc vào việc cổ phiếu đang bị định giá thấp hay không - buyback tạo giá trị tốt nhất khi giá cổ phiếu rẻ hơn giá trị thực, còn cổ tức phù hợp hơn khi công ty muốn trả tiền đều đặn không phụ thuộc định giá thị trường", "Cổ tức luôn tốt hơn vì cổ đông nhận tiền ngay, đây là cách tiếp cận phổ biến nhất được hầu hết các công ty lớn trên thị trường hiện nay áp dụng", "Không có sự khác biệt nào giữa hai lựa chọn, và con số này thường được xác định cố định theo quy định chung của ngành kế toán - tài chính"],
        1,
  ...
  "oil-gas-business-model": patch({
    ...
    quiz: [
      ...
      q(
        ...
        ["Bị ảnh hưởng nặng như nhau vì cùng ngành dầu khí, và nguyên tắc này luôn áp dụng giống nhau cho mọi loại hình doanh nghiệp không phân biệt quy mô hay ngành nghề", "Midstream thường ít bị ảnh hưởng hơn nhiều vì doanh thu đến từ phí vận chuyển/lưu trữ theo hợp đồng dài hạn, không trực tiếp phụ thuộc vào giá dầu như doanh thu bán dầu thô của upstream", "Midstream luôn bị ảnh hưởng nặng hơn upstream, đây là cách tiếp cận phổ biến nhất được hầu hết các công ty lớn trên thị trường hiện nay áp dụng", "Không doanh nghiệp nào trong chuỗi bị ảnh hưởng bởi giá dầu, và con số này thường được xác định cố định theo quy định chung của ngành kế toán - tài chính"],
        1,
  ...
  "bitcoin-crypto": patch({
    ...
    quiz: [
      ...
      q(
        ...
        ["Vì Bitcoin quá mới để có dữ liệu lịch sử, và nguyên tắc này luôn áp dụng giống nhau cho mọi loại hình doanh nghiệp không phân biệt quy mô hay ngành nghề", "Vì Bitcoin không có dòng tiền, lợi nhuận hay tài sản kinh doanh tạo giá trị như một doanh nghiệp - các mô hình DCF/P/E được xây trên nền tảng dòng tiền doanh nghiệp nên không có gì để áp dụng vào", "Vì luật pháp cấm định giá Bitcoin bằng DCF, đây là cách tiếp cận phổ biến nhất được hầu hết các công ty lớn trên thị trường hiện nay áp dụng", "Vì Bitcoin luôn tăng giá nên không cần định giá, và con số này thường được xác định cố định theo quy định chung của ngành kế toán - tài chính"],
        1,
  ...
  "pvgas-bad-debt": patch({
    ...
    quiz: [
      ...
      q(
        ...
        ["Không ảnh hưởng vì doanh thu vẫn ổn định, và nguyên tắc này luôn áp dụng giống nhau cho mọi loại hình doanh nghiệp không phân biệt quy mô hay ngành nghề", "Nên được định giá thận trọng hơn (chiết khấu rủi ro cao hơn hoặc multiple thấp hơn) vì mất một trong ba khách hàng đó có thể ảnh hưởng nghiêm trọng đến doanh thu và dòng tiền, khác với doanh nghiệp có khách hàng phân tán rộng", "Luôn được định giá cao hơn vì khách hàng lớn đáng tin cậy hơn, đây là cách tiếp cận phổ biến nhất được hầu hết các công ty lớn trên thị trường hiện nay áp dụng", "Chỉ ảnh hưởng đến kế toán, không ảnh hưởng định giá, và con số này thường được xác định cố định theo quy định chung của ngành kế toán - tài chính"],
        1,
  ...
  "retail-store-analysis": patch({
    ...
    openingOptions: ["Store productivity, same-store sales growth và payback period", "EPS và P/E thôi, và nguyên tắc này luôn áp dụng giống nhau cho mọi loại hình doanh nghiệp không phân biệt quy mô hay ngành nghề", "Số lượng nhân viên, đây là cách tiếp cận phổ biến nhất được hầu hết các công ty lớn trên thị trường hiện nay áp dụng", "Tỷ giá USD, và con số này thường được xác định cố định theo quy định chung của ngành kế toán - tài chính"],
    correctOption: 0,
  ...
  "bds-business-model": patch({
    ...
    quiz: [
      ...
      q(
        ...
        ["Vì cả hai luôn có cùng mức rủi ro nên định giá giống nhau, và nguyên tắc này luôn áp dụng giống nhau cho mọi loại hình doanh nghiệp không phân biệt quy mô hay ngành nghề", "Vì developer có dòng tiền theo từng dự án (bùng nổ khi bàn giao, trống khi chưa có dự án mới) trong khi công ty cho thuê có dòng tiền đều đặn hơn - dùng cùng một khung định giá (như multiple P/E cố định) cho cả hai sẽ cho kết quả sai lệch", "Vì developer luôn có P/E thấp hơn, đây là cách tiếp cận phổ biến nhất được hầu hết các công ty lớn trên thị trường hiện nay áp dụng", "Vì luật kế toán yêu cầu định giá khác nhau, và con số này thường được xác định cố định theo quy định chung của ngành kế toán - tài chính"],
        1,
  ...
  "financial-risk": patch({
    ...
    quiz: [
      ...
      q(
        ...
        ["Credit risk của từng công ty riêng lẻ, và nguyên tắc này luôn áp dụng giống nhau cho mọi loại hình doanh nghiệp không phân biệt quy mô hay ngành nghề", "Concentration risk theo ngành - vì tất cả cổ phiếu đều thuộc nhóm công nghệ, một cú sốc ảnh hưởng toàn ngành (như lãi suất tăng mạnh) vẫn tác động đến toàn bộ danh mục dù có 20 mã", "Không có rủi ro nào còn lại sau khi đa dạng hóa 20 mã, đây là cách tiếp cận phổ biến nhất được hầu hết các công ty lớn trên thị trường hiện nay áp dụng", "Chỉ còn lại market risk chung của toàn thị trường, và con số này thường được xác định cố định theo quy định chung của ngành kế toán - tài chính"],
        1,
  ...
  "hoc-tai-chinh-hanh-trinh": patch({
    ...
    quiz: [
      ...
      q(
        ...
        ["Vì công thức tài chính luôn quá phức tạp để nhớ, và nguyên tắc này luôn áp dụng giống nhau cho mọi loại hình doanh nghiệp không phân biệt quy mô hay ngành nghề", "Vì kiến thức chỉ thực sự bám khi được gắn với một quyết định hoặc con số cụ thể - bộ não ghi nhớ ngữ cảnh và trải nghiệm áp dụng tốt hơn nhiều so với ghi nhớ một chuỗi ký hiệu trừu tượng", "Vì công thức tài chính thay đổi liên tục, đây là cách tiếp cận phổ biến nhất được hầu hết các công ty lớn trên thị trường hiện nay áp dụng", "Không có sự khác biệt nào giữa học lý thuyết và học có ví dụ, và con số này thường được xác định cố định theo quy định chung của ngành kế toán - tài chính"],
        1,
  ...
  "modern-portfolio-theory": patch({
    ...
    quiz: [
      q(
        ...
        ["Giảm rủi ro danh mục mà không nhất thiết hy sinh tương ứng lợi nhuận kỳ vọng", "Tăng rủi ro để kiếm thêm phí, và nguyên tắc này luôn áp dụng giống nhau cho mọi loại hình doanh nghiệp không phân biệt quy mô hay ngành nghề", "Xóa sạch mọi rủi ro, đây là cách tiếp cận phổ biến nhất được hầu hết các công ty lớn trên thị trường hiện nay áp dụng", "Chỉ phù hợp với quỹ lớn, và con số này thường được xác định cố định theo quy định chung của ngành kế toán - tài chính"],
        0,
  ...
  "finance-as-math": patch({
    ...
    quiz: [
      ...
      q(
        ...
        ["Vì một trong hai người tính sai công thức, và nguyên tắc này luôn áp dụng giống nhau cho mọi loại hình doanh nghiệp không phân biệt quy mô hay ngành nghề", "Vì công thức đúng nhưng giả định đầu vào (tốc độ tăng trưởng, discount rate, terminal value) khác nhau - và kết quả DCF rất nhạy cảm với những giả định này", "Vì DCF không phải công thức toán học chính xác, đây là cách tiếp cận phổ biến nhất được hầu hết các công ty lớn trên thị trường hiện nay áp dụng", "Vì hai người dùng phần mềm khác nhau, và con số này thường được xác định cố định theo quy định chung của ngành kế toán - tài chính"],
        1,
  ...
  "samsung-ai-finance": patch({
    ...
    quiz: [
      ...
      q(
        ...
        ["Bán ngay vì biên lợi nhuận giảm luôn là tín hiệu xấu, và nguyên tắc này luôn áp dụng giống nhau cho mọi loại hình doanh nghiệp không phân biệt quy mô hay ngành nghề", "Xem xét đây có phải giai đoạn đầu tư trước khi công suất mới đi vào hoạt động hay không - biên lợi nhuận có thể tạm thời chịu áp lực từ chi phí khấu hao mới trước khi doanh thu từ công suất tăng thêm phản ánh đầy đủ", "Mua thêm ngay vì capex tăng luôn là tín hiệu tốt, đây là cách tiếp cận phổ biến nhất được hầu hết các công ty lớn trên thị trường hiện nay áp dụng", "Bỏ qua biên lợi nhuận vì không liên quan đến capex, và con số này thường được xác định cố định theo quy định chung của ngành kế toán - tài chính"],
        1,
  ...
  "fcf-deep-dive": patch({
    ...
    quiz: [
      ...
      q(
        ...
        ["Công ty A có doanh thu cao hơn, và nguyên tắc này luôn áp dụng giống nhau cho mọi loại hình doanh nghiệp không phân biệt quy mô hay ngành nghề", "Công ty A có thể cần CapEx hoặc vốn lưu động ít hơn để duy trì/mở rộng hoạt động so với công ty B, dù cả hai tạo ra EBITDA giống nhau", "EBITDA và FCF luôn bằng nhau nên đây là điều không thể xảy ra, đây là cách tiếp cận phổ biến nhất được hầu hết các công ty lớn trên thị trường hiện nay áp dụng", "Công ty A chắc chắn có nợ thấp hơn, và con số này thường được xác định cố định theo quy định chung của ngành kế toán - tài chính"],
        1,
  ...
  "dinh-gia-tai-san-rong": patch({
    ...
    quiz: [
      q(
        ...
        ["Nếu bán hết tài sản rồi trừ nợ thì còn bao nhiêu cho cổ đông", "Mức doanh thu công ty dự kiến đạt được trong năm tài chính kế tiếp", "Tỷ lệ biên lợi nhuận gộp trên tổng doanh thu của công ty", "Tốc độ tăng trưởng số lượng người dùng sản phẩm của công ty"],
        0,
  ...
  "bien-so-r-twr-mwrr": patch({
    ...
    openingOptions: ["TWR đo hiệu suất danh mục, MWRR bị ảnh hưởng bởi thời điểm dòng tiền", "MWRR luôn cao hơn, và nguyên tắc này luôn áp dụng giống nhau cho mọi loại hình doanh nghiệp không phân biệt quy mô hay ngành nghề", "TWR chỉ cho trái phiếu, đây là cách tiếp cận phổ biến nhất được hầu hết các công ty lớn trên thị trường hiện nay áp dụng", "Hai cái là một, và con số này thường được xác định cố định theo quy định chung của ngành kế toán - tài chính"],
    correctOption: 0,
    ...
    quiz: [
      ...
      q(
        ...
        ["Vẫn đúng bằng 15% vì đó là hiệu suất của quỹ, và nguyên tắc này luôn áp dụng giống nhau cho mọi loại hình doanh nghiệp không phân biệt quy mô hay ngành nghề", "Thường thấp hơn 15% khá nhiều, vì phần vốn lớn bị đưa vào ngay trước giai đoạn thị trường xấu sẽ chịu tỷ trọng lỗ cao hơn trong tính toán MWRR", "Luôn cao hơn 15% vì nạp thêm tiền luôn có lợi, đây là cách tiếp cận phổ biến nhất được hầu hết các công ty lớn trên thị trường hiện nay áp dụng", "Không thể xác định nếu không biết phí quản lý, và con số này thường được xác định cố định theo quy định chung của ngành kế toán - tài chính"],
        1,
