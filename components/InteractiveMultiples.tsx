...
export default function InteractiveMultiples() {
  ...
  const [ebitda, setEbitda] = useState(200);   // nghìn request/phút ở giờ cao điểm
  const [multiple, setMultiple] = useState(8); // hệ số so với dịch vụ tham chiếu
  const [netDebt, setNetDebt] = useState(300); // dung lượng giữ lại làm dự phòng
  const [shares, setShares] = useState(50);    // số vùng/cụm triển khai
    ...
    <div className="bg-white rounded-3xl border border-stone-100 p-6 space-y-5 dark:bg-stone-900 dark:border-stone-800">
      ...
      <div className="space-y-4">
        <div>
          ...
          <input type="range" min={50} max={600} step={10} value={ebitda} onChange={(e) => setEbitda(+e.target.value)} className="w-full" aria-label={t.multiplesCalc.ebitdaLabel} />
        </div>
        <div>
          <div className="flex justify-between text-sm mb-2">
