...
export default function MessageUserButton({ targetUserId }: { targetUserId: string }) {
  ...
  if (connection === undefined) {
    return (
      <button
        disabled
        className="inline-flex items-center gap-1.5 text-sm font-semibold px-4 py-2 rounded-xl border border-line text-ink-faint"
      >
  ...
  return (
    <button
      ...
      disabled={busy}
      className="inline-flex items-center gap-1.5 text-sm font-bold px-4 py-2 rounded-xl bg-surface-invert text-ink-invert hover:opacity-90 transition-opacity disabled:opacity-60"
    >
