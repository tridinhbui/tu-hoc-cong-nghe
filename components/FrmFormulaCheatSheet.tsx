    ...
    <div className="space-y-8">
        ...
        <div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="rounded-full border border-stone-700 bg-stone-800 px-2.5 py-1 text-[10px] font-black uppercase tracking-widest text-stone-300">
              {FRM_FORMULAS_DATA.length} công thức
            </span>
          </div>
          <h2 className="mt-2 text-2xl font-black text-white">FRM Part I &amp; II Formula Sheet</h2>
          <p className="mt-1 max-w-xl text-xs text-stone-300 sm:text-sm">
            Công thức theo đúng mười môn của GARP, kèm giải thích từng ký hiệu và một ví dụ tính
            bằng số - phần thường bị bỏ qua nhưng lại là chỗ công thức trở nên dùng được.
          </p>
        </div>
      ...
      <div className="space-y-4">
        <div className="relative max-w-md">
          ...
          <input
            type="text"
            placeholder="Tìm công thức (VD: VaR, LCR, Sharpe, DV01, EL...)"
            value={searchQuery}
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            Tất cả ({FRM_FORMULAS_DATA.length})
          </button>
      ...
      {filtered.length > 0 ? (
        ...
        <p className="py-12 text-center text-sm text-stone-500 dark:text-stone-400">
          Không có công thức nào khớp với từ khoá này.
        </p>
      )}
    </div>
  );
