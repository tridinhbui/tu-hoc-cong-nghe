  ...
  return (
    <div className="pointer-events-auto absolute inset-x-3 bottom-36 z-10 max-h-[62vh] overflow-y-auto rounded-2xl border border-stone-700 bg-stone-900/95 p-4 shadow-2xl backdrop-blur sm:inset-x-auto sm:bottom-4 sm:left-4 sm:w-[26rem]">
      <div className="mb-2 flex items-start justify-between gap-2">
        <div>
          ...
          <p className="mt-0.5 text-[11px] text-stone-400">
            Một bút toán chạm vào cả ba bảng cùng lúc
          </p>
        </div>
        <button
          type="button"
      ...
      {impact && phase === "guess" && (
        <div className="mb-2 rounded-xl border border-cyan-500/40 bg-cyan-950/30 p-3">
          ...
          <p className="mt-1 text-[10px] text-stone-400">
            Tự trả lời trong đầu trước đã — rồi mới lật.
          </p>
          <button
            type="button"
      ...
      <div className="space-y-1.5">
        <Wall title="Kết quả kinh doanh" accent={accent}>
          <Row label="Doanh thu" value={after.incomeStatement.revenue} before={base.incomeStatement.revenue} reveal={reveal} />
        ...
        <Wall title="Lưu chuyển tiền tệ" accent={accent}>
          <Row label="LNST" value={after.cashFlow.netIncome} before={base.cashFlow.netIncome} reveal={reveal} />
          <Row label="+ Khấu hao (không chi tiền)" value={after.cashFlow.depreciation} before={base.cashFlow.depreciation} reveal={reveal} />
          <Row label="± Vốn lưu động" value={after.cashFlow.workingCapitalChange} before={base.cashFlow.workingCapitalChange} reveal={reveal} />
          <Row label="CFO" value={after.cashFlow.cfo} before={base.cashFlow.cfo} reveal={reveal} />
          <Row label="CFI" value={after.cashFlow.cfi} before={base.cashFlow.cfi} reveal={reveal} />
          <Row label="CFF" value={after.cashFlow.cff} before={base.cashFlow.cff} reveal={reveal} />
          <Row label="Tiền tăng/giảm trong kỳ" value={after.cashFlow.netChange} before={base.cashFlow.netChange} reveal={reveal} />
        </Wall>
        ...
        <Wall title="Bảng cân đối" accent={accent}>
          ...
          <p className="mt-1 text-center text-[10px] font-bold text-emerald-400">
            Hai vế lệch nhau: {after.balanceCheck} ✓
          </p>
        </Wall>
      </div>
