...
export function parseMigration(sql) {
  ...
  return { tables, columns, functions, indexes, policies, triggers };
}
...
`;
  ...
  console.log(
    `Đã ghi ${SQL_OUT} - ${functions.size} function, ${indexes.size} index, ${policies.size} policy, ${triggers.size} trigger.`
  );
