-- Fix all Postgres Database errors reported in Supabase Log Explorer:
-- 1. Fix Error 23514: game_sessions_game_type_check violation
-- 2. Fix Error 42703: column user_profiles.coins does not exist & notification_preferences.weekly_digest_enabled does not exist
...
-- 2. Add missing columns to user_profiles and notification_preferences if not present
ALTER TABLE public.user_profiles ADD COLUMN IF NOT EXISTS coins integer DEFAULT 0;
ALTER TABLE public.notification_preferences ADD COLUMN IF NOT EXISTS weekly_digest_enabled boolean DEFAULT false;
