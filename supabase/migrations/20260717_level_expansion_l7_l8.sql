-- lib/levels.ts raised L5 (1000->1200) and L6 (1500->2000) XP thresholds and
-- added two new tiers (L7 "Chuyên gia Tài chính" 3200xp, L8 "Bậc thầy Tài
-- chính" 5000xp). get_level_stats hardcodes the old 6-level XP bucket
-- ranges, so it must be updated in lockstep or the level roadmap's
-- per-level user counts would silently stop matching what lib/levels.ts
-- (and thus every level-up check) actually computes.
